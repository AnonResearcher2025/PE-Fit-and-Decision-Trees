...
coefficients = [
    (0.214, -0.222, -0.16, 0.051, 0.111, 0),
    (-0.011, 0.28, 0.041, -0.127, 0.082, 0),
    (0.019, 0.262, -0.012, -0.022, 0.056, 0),
    (-0.09, 0.11, -0.13, -0.17, 0.23, 0),
    (-0.06, 0.11, -0.05, 0.01, 0.11, 0),
    (-0.11, 0.25, -0.01, 0, 0, 0) ]
...
for idx, (a, b, c, d, e, f) in enumerate(coefficients):
    # Default
    n = 300
    ...
    Z_highPE = a * P_highPE + b * E_highPE + c * P_highPE**2 + d * E_highPE**2 + e * P_highPE * E_highPE + f
...
def build_and_store_models(dataset_type, dataset):
    models[dataset_type] = {}
    cbar_vals[dataset_type] = {}  # Ensure each dataset type has its own sub-dictionary
...
def plot_model(ax, model, X, y, lbx, ubx, lby, uby, lb_out, ub_out, inc, colormap='bwr', scatter=True):
